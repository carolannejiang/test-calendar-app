...
Candidates need only the site URL: the start gate sends the password they enter to `POST /api/open`, and the matching test replaces the `scenario.js` calendar in that browser until `#reset`. The `scenario.js` password still opens the default scenario, so no test may reuse it (the reviewer panel refuses such a password). The gate is shown on every hosted deployment, even when `scenario.js` sets no `openPasswordHash`; in that case an empty password opens the default scenario. Activating a test needs working browser storage, because the page reloads into it; a browser that blocks storage is told so at the gate. `/api/open` is public and not rate limited by the app itself, so rely on the platform's limits and use activation passwords that are not guessable. Reviewer-created tests share the default scenario's week, visible hours, timezone, and instructions.

Tests are stored as `tests/<slug>.json` in the private Blob store, where the slug is derived from the title (and must be unique, as must the activation password). The slug also appears in reviewer links such as `/?test=<slug>#review`, which require the reviewer password; it never grants candidates access. Each time a starting calendar is saved with different events the test's revision increases. The revision is part of the scenario name (`Title (rev N)`), so drafts, submission locks, and change comparisons are always tied to the calendar the candidate actually started from; submissions made against an older revision show as a different scenario. Changing only the password keeps the revision. Every update names the revision the reviewer loaded and the storage record it read, so if someone else saved the same test in the meantime the update is refused with a conflict and the page must be reloaded; two different calendars can never share a revision number.
...
The submission lock is also stored per scenario name, so changing the scenario unlocks returning browsers with a fresh candidate ID. Opening the page with `#reset` clears the draft, the start gate, and the lock, which issues a new candidate ID. If a reviewer-created test is active, `#reset` first drops the test and reloads with `#reset` kept, so the default scenario's draft, gate, and lock are cleared as well.
