...
const { redis, cors, fail, HASH_KEY } = require("./_store");
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (json.length > 200_000) return res.status(413).json({ ok: false, error: "Submission too large" });
    const t = typeof p.t === "string" && !isNaN(Date.parse(p.t)) ? p.t : new Date().toISOString();
    const id = `${p.c}-${Date.parse(t)}`;
    await redis("HSET", HASH_KEY, id, JSON.stringify({ ...p, t, id, receivedAt: new Date().toISOString() }));
